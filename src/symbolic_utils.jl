function make_substitutions(t, f_var)
    ...
    return (symbol = sym_name, var = names[1], var_p = names[2], var_ss = names[3],
            markov_t = f_var(t) => names[1], markov_tp1 = f_var(t + 1) => names[2],
            markov_inf = f_var(Inf) => names[3], tp1_to_var = names[2] => names[1],
