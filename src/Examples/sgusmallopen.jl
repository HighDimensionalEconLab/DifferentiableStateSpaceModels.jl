...
function sgusmallopen()
    ...
    H = [d - (1 + exp(Lr)) * Ld + exp(GDP) - exp(c) - exp(i) -
    ...
    ȳ = [d_ss ~ d_bar, c_ss ~ log(cstar), h_ss ~ log(hstar), GDP_ss ~ log(GDPstar),
          i_ss ~ log(istar), k_ss ~ log(kstar), λ_ss ~ log(λstar), tb_ss ~ log(tbstar),
          ca_ss ~ 0, riskpremium_ss ~ 0, r_ss ~ log((1 - β) / β)]
