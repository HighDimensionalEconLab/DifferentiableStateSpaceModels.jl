...
solve(f, g, u0, tspan, p = nothing, alg = GeneralTimeVarying(); h = (u,p,t) -> u, D = nothing, noise = nothing, observables = nothing, save_everystep  = true, save_posteriors = true, calculate_logpdf = true, simulate_observation_noise = false) = _solve(alg, noise, observables, f, g, h, D, u0, tspan, p, save_everystep, save_posteriors, calculate_logpdf, simulate_observation_noise)
...
function _solve(alg::GeneralTimeVarying, noise, observables, f, g, h, D, u0, tspan, p, save_everystep, save_posteriors, calculate_logpdf, simulate_observation_noise)
    T = tspan[2]
    @assert tspan[1] == 0
...
############################################
# LTI Specializations
solve(A::AbstractMatrix, B::AbstractMatrix, C::AbstractMatrix, D, u0, tspan, alg = LTI(); noise = nothing, observables = nothing) = _solve(alg, noise, observables, A, B, C, D, u0, tspan)

# LTI with observables and without conditioning on noise = Kalman Filter
function _solve(alg::LTI, noise::Nothing, observables, A::AbstractMatrix, B::AbstractMatrix, C::AbstractMatrix, D::TuringDiagMvNormal, u0, tspan)
...
function _solve(alg::LTI, noise, observables, A::AbstractMatrix, B::AbstractMatrix, C::AbstractMatrix, D, u0, tspan)
    T = tspan[2]
    @assert tspan[1] == 0
    @assert length(noise) == T
...
function _solve(alg::LTI, noise, observables::Nothing, A::AbstractMatrix, B::AbstractMatrix, C::AbstractMatrix, D, u0, tspan)
    T = tspan[2]
    @assert tspan[1] == 0
    @assert length(noise) == T
...
function _solve(alg::LTILikelihood, noise::Nothing, observables, A::AbstractMatrix, B::AbstractMatrix, C::AbstractMatrix, D::TuringDiagMvNormal, u0, tspan)
    # hardcoded right now for tspan = (0, T) for T+1 points
    T = tspan[2]
    @assert tspan[1] == 0
    @assert length(observables) == T # i.e. we do not calculate the likelihood of the initial condition
...
function _solve(alg::LTILikelihood, noise, observables, A::AbstractMatrix, B::AbstractMatrix, C::AbstractMatrix, D, u0, tspan)
    T = tspan[2]
    @assert tspan[1] == 0
    @assert length(noise) == T
...
solve(A_0, A_1, A_2, B, C_0, C_1, C_2, D, u0, tspan, alg = QTI(); noise = nothing, observables = nothing) = _solve(alg, noise, observables, A_0, A_1, A_2, B, C_0, C_1, C_2, D, u0, tspan)
...
=#
quad(A::AbstractArray{<:Number,3}, x) = @matmul c[l] := sum(j) (@matmul [l,j] := sum(k) A[l,j,k] * x[k]) * x[j]
...
# QTI with noise and observables is a joint likelihood
function _solve(alg::QTILikelihood, noise, observables, A_0, A_1, A_2, B, C_0, C_1, C_2, D, u0, tspan)
    T = tspan[2]
    @assert tspan[1] == 0
    @assert length(noise) == T
...
# QTI with noise and observables is a joint likelihood
function _solve(alg::QTI, noise, observables::Nothing, A_0, A_1, A_2, B, C_0, C_1, C_2, D, u0, tspan)
    T = tspan[2]
    @assert tspan[1] == 0
    @assert length(noise) == T
...
solve(sol::FirstOrderPerturbationSolution, u0, tspan, alg = LTILikelihood(); kwargs...) = solve(sol.A, sol.B, sol.C, sol.D, u0, tspan, alg;kwargs...)

solve(sol::SecondOrderPerturbationSolution, u0, tspan, alg = QTILikelihood(); kwargs...) = solve(sol.A_0, sol.A_1, sol.A_2, sol.B, sol.C_0, sol.C_1, sol.C_2, sol.D, u0, tspan, alg;kwargs...)
...
function save_model_results(file_prefix, path, sol::FirstOrderPerturbationSolution, ϵ, z)
    try
        ...
        CSV.write(joinpath(path, "$(file_prefix)_A.csv"), DataFrame(sol.A), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_B.csv"), DataFrame(sol.B), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_C.csv"), DataFrame(sol.C), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_D.csv"), DataFrame(column_name = sol.D.σ), writeheader = false) # Extract the diagonal
        # In addition, save ϵ and z with different file names
        CSV.write(joinpath(path, "$(file_prefix)_noise.csv"), DataFrame(transpose(hcat(ϵ...))), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_observables.csv"), DataFrame(transpose(hcat(z...))), writeheader = false)
        # Save the ergodic distribution for first order
        CSV.write(joinpath(path, "$(file_prefix)_ergodic.csv"), DataFrame(sol.x_ergodic.C.L * sol.x_ergodic.C.U), writeheader = false)
    catch err
...
function save_model_results(file_prefix, path, sol::SecondOrderPerturbationSolution, ϵ, z)
    try
        ...
        CSV.write(joinpath(path, "$(file_prefix)_A_0.csv"), DataFrame(column_name = sol.A_0), writeheader = false) # Vector
        CSV.write(joinpath(path, "$(file_prefix)_A_1.csv"), DataFrame(sol.A_1), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_A_2.csv"), DataFrame(column_name = vec(sol.A_2)), writeheader = false) # Save as a vector, reshape it when loading
        CSV.write(joinpath(path, "$(file_prefix)_B.csv"), DataFrame(sol.B), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_C_0.csv"), DataFrame(column_name = sol.C_0), writeheader = false) # Vector
        CSV.write(joinpath(path, "$(file_prefix)_C_1.csv"), DataFrame(sol.C_1), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_C_2.csv"), DataFrame(column_name = vec(sol.C_2)), writeheader = false) # Save as a vector, reshape it when loading
        CSV.write(joinpath(path, "$(file_prefix)_D.csv"), DataFrame(column_name = sol.D.σ), writeheader = false) # Extract the diagonal
        # In addition, save ϵ and z with different file names
        CSV.write(joinpath(path, "$(file_prefix)_noise.csv"), DataFrame(transpose(hcat(ϵ...))), writeheader = false)
        CSV.write(joinpath(path, "$(file_prefix)_observables.csv"), DataFrame(transpose(hcat(z...))), writeheader = false)
