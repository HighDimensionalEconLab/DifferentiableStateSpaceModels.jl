...
function calculate_steady_state!(m::PerturbationModel, c, settings, p, solver)
    ...
    try
        if !isnothing(m.mod.ȳ!) && !isnothing(m.mod.x̄!) # use closed form if possible
            m.mod.ȳ!(c.y, p)
            m.mod.x̄!(c.x, p)
            isnothing(m.mod.ȳ_p!) || foreach(s -> m.mod.ȳ_p!(c.y_p, Val(s), p), m.mod.p_symbols)
            isnothing(m.mod.x̄_p!) || foreach(s -> m.mod.x̄_p!(c.y_p, Val(s), p), m.mod.p_symbols)
