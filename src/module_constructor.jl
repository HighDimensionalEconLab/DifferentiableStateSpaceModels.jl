function FirstOrderPerturbationModel(mod::Module)
    return FirstOrderPerturbationModel(; mod.n, mod.n_y, mod.n_x, mod.n_p,
                                                  mod.n_ϵ, mod.n_z, η = copy(mod.η),
                                                  Q = copy(mod.Q), mod.Γ!, mod.Γ_p!, mod.Ω!,
                                                  mod.Ω_p!, mod.H!, mod.H_yp!, mod.H_y!,
                                                  mod.H_xp!, mod.H_x!, mod.H_yp_p!,
                                                  mod.H_y_p!, mod.H_xp_p!, mod.H_x_p!,
                                                  mod.H_p!, mod.Ψ!, mod.H̄!, mod.H̄_w!,
                                                  mod.ȳ_iv!, mod.x̄_iv!, mod.ȳ!, mod.x̄!,
                                                  mod.ȳ_p!, mod.x̄_p!, mod.steady_state!,
...
function SecondOrderPerturbationModel(mod::Module)
    return SecondOrderPerturbationModel(; mod.n, mod.n_y, mod.n_x, mod.n_p,
                                                   mod.n_ϵ, mod.n_z, η = copy(mod.η),
                                                   Q = copy(mod.Q), mod.Γ!, mod.Γ_p!,
                                                   mod.Ω!, mod.Ω_p!, mod.H!, mod.H_yp!,
                                                   mod.H_y!, mod.H_xp!, mod.H_x!,
                                                   mod.H_yp_p!, mod.H_y_p!, mod.H_xp_p!,
                                                   mod.H_x_p!, mod.H_p!, mod.Ψ!, mod.H̄!,
                                                   mod.H̄_w!, mod.ȳ_iv!, mod.x̄_iv!,
                                                   mod.ȳ!, mod.x̄!, mod.ȳ_p!, mod.x̄_p!,
                                                   mod.steady_state!,
                                                   functions_type = mod.functions_type(),
                                                   select_p_ss_hash = copy(mod.select_p_ss_hash),
                                                   select_p_f_ss_hash = copy(mod.select_p_f_ss_hash),
                                                   select_p_perturbation_hash = copy(mod.select_p_perturbation_hash),
                                                   select_p_f_perturbation_hash = copy(mod.select_p_f_perturbation_hash),
                                                   mod.allocate_solver_cache, mod.Ψ_x!,
                                                   mod.Ψ_y!, mod.Ψ_xp!, mod.Ψ_yp!, mod.Ψ_p!)
end
...
```
"""
macro include_example_module(model_generator, order = 1, functions_type = DifferentiableStateSpaceModels.DenseFunctions())
    quote
        function_name = "$(Base.nameof($(model_generator)))"  # strips out module name
        local model_name =  ($functions_type isa DifferentiableStateSpaceModels.SparseFunctions) ? "$($model_generator)_$($order)_sparse" :  "$($model_generator)_$($order)"
        local model_filename = "$(model_name).jl"
        local save_module_function = ($order == 1) ? DifferentiableStateSpaceModels.save_first_order_module : DifferentiableStateSpaceModels.save_second_order_module        
        local model_cache_path = joinpath(DifferentiableStateSpaceModels.default_model_cache_location(), model_filename)
        if !isfile(model_cache_path)
             local H, mod_vals = $model_generator()
             save_module_function(H; model_name, functions_type = $functions_type, mod_vals...)
             include(model_cache_path)
        elseif !isdefined(Main, Symbol(model_name))
             include(model_cache_path)
        end
        local mod =  getfield(Main, Symbol(model_name))
        ($order == 1) ? DifferentiableStateSpaceModels.FirstOrderPerturbationModel(mod) : DifferentiableStateSpaceModels.SecondOrderPerturbationModel(mod)
    end |> esc
end
