...
Base.show(io::IO, ::MIME"text/plain", m::ThreadLocalCache) = print(io,
  "ThreadLocalCache with $(m.caches.count) elements\n")
...
```
"""
function get_threadsafe_cache(cache::ThreadLocalCache, m::AbstractPerturbationModel, p, p_f, settings = PerturbationSolverSettings())
    ...
    hash_match_ss = (hash(get_hash_subset(m.select_p_ss_hash, p)) == c.p_ss_hash) &&
                    (hash(get_hash_subset(m.select_p_f_ss_hash, p_f)) == c.p_f_ss_hash)

    hash_match_perturbation = (hash(get_hash_subset(m.select_p_perturbation_hash, p)) ==
                            c.p_perturbation_hash) &&
                            (hash(get_hash_subset(m.select_p_f_perturbation_hash, p_f)) ==
                            c.p_f_perturbation_hash)
    if settings.print_level >= 2 && (hash_match_all || hash_match_ss || hash_match_perturbation)
        println("Cache hit matching: all = $hash_match_all, steady-state = $hash_match_ss, perturbation = $hash_match_perturbation")
