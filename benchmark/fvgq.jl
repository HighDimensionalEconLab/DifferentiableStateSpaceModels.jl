...
isdefined(Main, :FVGQ20) || include(joinpath(pkgdir(DifferentiableStateSpaceModels), "test/generated_models/FVGQ20.jl"))
...
# Basic Steady State
p_d_fvgq = (β=0.998, h=0.97, ϑ=1.17, κ=9.51, α=0.21, θp=0.82, χ=0.63, γR=0.77, γy=0.19, γΠ=1.29, Πbar=1.01, ρd=0.12, ρφ=0.93, ρg=0.95, g_bar=0.3, σ_A=exp(-3.97), σ_d=exp(-1.51), σ_φ=exp(-2.36), σ_μ=exp(-5.43), σ_m=exp(-5.85), σ_g=exp(-3.0), Λμ=3.4e-3, ΛA=2.8e-3)
...
# fvgq
const FVGQ["fvgq"]["first_order"] =
    @benchmarkable generate_perturbation($m_fvgq, $p_d_fvgq, $p_f_fvgq, Val(1); cache = $c_fvgq_1)
const FVGQ["fvgq"]["first_order_p"] =
    @benchmarkable generate_perturbation_derivatives!($m_fvgq, $p_d_fvgq, $p_f_fvgq, $c_fvgq_1_p)
const FVGQ["fvgq"]["second_order"] =
    @benchmarkable generate_perturbation($m_fvgq, $p_d_fvgq, $p_f_fvgq, Val(2); cache = $c_fvgq_2)
const FVGQ["fvgq"]["second_order_p"] =
    @benchmarkable generate_perturbation_derivatives!($m_fvgq, $p_d_fvgq, $p_f_fvgq, $c_fvgq_2_p)
const FVGQ["fvgq"]["second_order_no_cache"] =
    @benchmarkable generate_perturbation($m_fvgq, $p_d_fvgq, $p_f_fvgq, Val(2)) # just to see how long the cache takes
