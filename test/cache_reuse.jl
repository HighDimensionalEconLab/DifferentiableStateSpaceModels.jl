# removed cache reuse for now.  Can add back later.

# using DifferentiableStateSpaceModels: all_equal_struct, check_cache
# @testset "Checks cache reuse first-order" begin
#     settings = PerturbationSolverSettings(; print_level = 2)
#     settings_no_cache =
#         PerturbationSolverSettings(; print_level = 2, use_solution_cache = false)
...
#     c_1 = check_cache(cache, m, p, p_f).c |> deepcopy
#     sol_2 = generate_perturbation(m, p; p_f, settings, cache)
#     c_2 = check_cache(cache, m, p, p_f).c |> deepcopy
#     sol_3 = generate_perturbation(m, p; p_f, settings = settings_no_cache, cache)
#     c_3 = check_cache(cache, m, p, p_f).c |> deepcopy
...
#     c_0 = check_cache(cache, m, p, p_f).c |> deepcopy # before calling anything
