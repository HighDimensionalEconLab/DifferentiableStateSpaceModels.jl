...
function likelihood_test_joint_first_sol(p_d_input, p_f, ϵ, x0, m, tspan, z)
    p_d = (α=p_d_input[1], β=p_d_input[2])
    return solve(generate_perturbation(m, p_d, p_f), x0, tspan; observables = z, noise = ϵ).logpdf
end
...
@testset "Gradients, generate_perturbation + likelihood, 1st order" begin
    ...
    res = gradient((p_d_input, ϵ) -> likelihood_test_joint_first(p_d_input, p_f, ϵ, x0, m, tspan, z), p_d_input, ϵ)
    ...
    res2 = gradient(
        (p_d_input, ϵ) -> likelihood_test_joint_first_sol(p_d_input, p_f, ϵ, x0, m, tspan, z),
        p_d_input,
        ϵ,
    )
    @test res[1] ≈ res2[1]
