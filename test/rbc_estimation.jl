using DifferentiableStateSpaceModels, LinearAlgebra, Optim, Test, TimerOutputs, Turing, Zygote, BenchmarkTools, StatsPlots, TensorBoardLogger, Logging
...
chain = sample(turing_model, Gibbs(HMC(ϵ_leapfrog, n_depth, :α, :β), HMC(ϵ_leapfrog, n_depth, :ϵ_draw)), n_samples; progress = true, callback)
...
chain = sample(turing_model, NUTS(n_adapts,δ; max_depth), n_samples; progress = true, callback)